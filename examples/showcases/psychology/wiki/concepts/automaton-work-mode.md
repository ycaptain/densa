...
triggers: ["workplace meetings", "standup", "1:1s", "performance reviews", "any context with public competence demand"]
first_observed: 2026-04-02
last_observed: 2026-05-14
severity: medium
tags: [work, dissociation-light, derealisation-light, ifs-manager, automaton, half-volume]
aliases: ["watching myself work", "two-of-me", "automaton mode", "half-volume mode"]
status: active
compiled_against: 2
migration_history:
  - from: 1
...
# Automaton work-mode ("watching myself do it")

> [!important] Concept one-liner (v1 pattern)
> Sustained low-level *derealisation-flavoured* functional state at
> work: standup runs, 1:1s happen, performance reviews ship, but
...
- **Watching-self quality**: a stable observer position behind the
  ...
  me."
- **Diurnal pattern**: most intense during the workday; less
  pronounced (or differently shaped) at home, where the
  alternative is not "watching" but **withdrawing** (couch +
  zone-out TV; see contrast with [[avoidant-mother-contact]]).
- **External legibility**: nearly zero. The pattern is invisible
...
- **Protector function**: the foreman ([[inner-protector-stoic]])
  maintains daily functioning by allocating the "performing"
  layer to work demands while the "watching" layer carries the
  grief load without it bleeding into the performing layer. The
...
In an arc where avoidant-mother-contact is the per-week tracked
marker and the chest-band is the somatic anchor, work-mode is
**deliberately not** the primary intervention target. Reasons:

1. **Functional preservation matters.** Mark's professional role
   is part of what is holding family stability (mortgage, school
   for Theo) together. Disrupting the work-mode before the home /
   relational work has progressed would be iatrogenic.
2. **No external pressure for change.** Coworkers don't notice;
   no professional consequence is forcing a re-evaluation.
3. **Likely downstream resolution.** As the grief integrates and
   the protector re-deploys (see [[inner-protector-stoic]]
   §"Cross-session evolution"), work-mode is the pattern most
   likely to attenuate *as a side effect* rather than requiring
   direct work.

The clinical bet is that work-mode loosens 6-12 months after the
home / relational layer has loosened. If at the 12-month mark
work-mode persists at session-1 intensity, that would be an
indication to take it up directly.

## Cross-references

- [[inner-protector-stoic]] — the part doing this work.
- [[somatic-grief-containment]] — the somatic counterpart of the
  same containment.
- [[masked-depression]] — the broader presentation in which the
  half-volume framing is the lead self-frame.
- [[father-grief-arc]] — the arc this pattern is embedded in.
- [[ifs]] — Manager-part dynamics framework.
