...
status: active
tags: [psychiatry, medication, deferred, ssri, trazodone, sleep, bereavement]
aliases: ["medication arc", "psychiatry timeline", "han medication record"]
compiled_against: 2
migration_history:
  - from: 1
    to: 2
    on: 2026-05-26
    mode: in-place
    notes: 'type protocol → concept'
---

# Medication arc

> [!important] What this protocol tracks
> The longitudinal medication record across the psychiatry channel
> with Dr. Han. **Every psychiatry session adds a row, including
> "no change" sessions** (per L2 §"Ingest flow", step 7 — this is
> non-negotiable for continuity of the record). The arc is the
> single audit-able view of what was decided, when, why, and what
> was deliberately *not* done.

## Baseline snapshot (2026-04-23 — first visit)
...
> *Bereavement-related depressive episode with complicated-grief
...
- **2026-05-07 (next visit)**: trazodone start vs. continued hold,
  based on sleep trajectory across the two-week window.
- **Mid-July 2026 (≈12 weeks post-loss)**: SSRI question revisited
  regardless. Marker to watch: avoidance-cluster loosening
  ([[avoidant-mother-contact]]). If avoidance is partially
  resolved by mid-July, SSRI may not need to be on the table at
  all.
- **Ongoing**: monitor for trajectory toward
  [[complicated-grief]] / PGD consolidation if at 6 months
  post-loss the symptom cluster has not begun to integrate. CGT
  (Shear et al.) is the gold-standard reach if PGD consolidates.

## Cross-references

- [[2026-04-23-psychiatry-han-summary]] — the analysis backing
  this protocol's 2026-04-23 entry.
- [[psychiatrist-han]] — the prescribing clinician.
- [[complicated-grief]] — the construct guiding the conservative
  SSRI hold.
- [[masked-depression]] — the presentation shape this protocol's
  pacing is calibrated to.
- [[integrative-grief-therapy]] — the parallel psychotherapy
  channel; medication decisions sequence with process work.

## Notes on protocol semantics in this arc

- The protocol-page convention in this L2 is that **every
  psychiatry session writes one row**, even when the substantive
...
- A protocol row is **not** a clinical recommendation outside this
  case; it is the audit record of *one* clinician's reasoning at
  one moment, on one patient. Reading this protocol as
  generalised guidance for SSRI-deferral-in-bereavement would be
  a category error.
