...
> Consulting psychiatrist for [[father-grief-arc]]. Biological /
> DSM-5-TR-informed orientation with explicitly **conservative
> prescribing** stance — does not initiate medication in a first
> visit by default and is *especially* cautious in early
> bereavement contexts. Coordinates closely with the psychotherapy
> channel ([[therapist-reyes]]); does not duplicate process work.
...
- **Scale-grounded** but not scale-led: uses PHQ-9, PCL-5, GAD-7,
  ...
  *in clinical context* (PHQ-9 = 14 in bereavement is not the
  same animal as PHQ-9 = 14 in primary MDD).

## Themes / arcs this entity participates in

- [[medication-decision-arc]] — primary; this is the arc Han
  *runs*.
- [[father-grief-arc]] — secondary; Han is the psychiatric
  channel for the larger grief arc.

## Appearances

| Date | Session | Decision | Notes |
| --- | --- | --- | --- |
...
Even from one session, the prescribing stance has identifiable
markers a reader can recognise across psychiatric styles:
...
- **Separates the depression question from the sleep question.**
  ...
  *might* be addressed pharmacologically (trazodone) even while
  the SSRI question is being deliberately deferred. This
  decoupling lets the sleep-deprivation contribution to the
  apparent depressive load be assessed before deciding on
  primary antidepressant treatment.
- **Reads scales as risk-stratification, not as diagnosis.** The
  PCL-5 = 28 sub-threshold result is interpreted as a
  *dimension* of avoidance, not as a PTSD diagnosis question;
  the avoidance cluster is identified as the clinically
  actionable signal.
- **Validates the process work without doing it.** Han
...
- [[therapist-reyes]] — coordinated-care counterpart.
- [[medication-arc]] — the longitudinal protocol Han maintains.
- [[complicated-grief]] — the diagnostic construct guiding the
  conservative SSRI hold.
- [[masked-depression]] — the presentation shape calibrating the
  pacing.
