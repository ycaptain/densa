...
> **What about deciding which one to keep / adapt / remove?** See
> [`docs/setup.md`](../../docs/setup.md) §"Choosing or replacing the
> default domain". This page is the *what it looks like* tour; that
> section is the *what to do with it* matrix.

> [!warning] Schema versions on this page
> Only `research-papers/` (the active default under `domains/`) is on
> the current **`schema_version: 2`** vocabulary (`summaries/`,
> `overviews/`, `open-questions/`). The two showcases below
> (`workspace/` and `psychology/`) still use **v1** vocabulary
> (`analyses/`, `frameworks/`, `patterns/`, `themes/`, `questions/`)
> and will only resurface in the active wiki graph after you copy them
> into `domains/<X>/` and run
> `python _system/scripts/migrate_02_karpathy_vocab.py --apply`. Read
> them for L2 design inspiration, not as templates to drop unchanged.

## At a glance

| Domain | Lives at | Schema | Density | Anchor arc |
|---|---|---|---|---|
| [`research-papers`](../../domains/research-papers/) | `domains/` (active default) | **v2** | Light (6 page types) | 5-paper LLM-tutoring evidence arc (2024-2025) |
| [`workspace`](workspace/) | `examples/showcases/` (opt-in) | v1 | Medium (8 page types) | Q2 platform-migration decision arc + positive-pattern retrospective |
| [`psychology`](psychology/) | `examples/showcases/` (opt-in) | v1 | Heavy (11 page types) | 6-week father-grief therapy + psychiatry arc |
...
The validator skips this directory for wikilink resolvability checks
and frontmatter freshness warnings; copy a showcase into `domains/`
if you want to start ingesting against its schema (mechanics:
[`docs/setup.md`](../../docs/setup.md) §"Choosing or replacing the
default domain").
...
on the current `schema_version: 2`. The most common adoption path:
keep this domain, replace the raws with your own papers, leave the
schema alone.
...
synthesised paper lets us show the full ingest → summary → concept
→ overview → open-question chain on a single self-contained example.
...
| [`wiki/summaries/2024-anthropic-sparse-autoencoders-summary.md`](../../domains/research-papers/wiki/summaries/2024-anthropic-sparse-autoencoders-summary.md) | A canonical 1:1 summary — claim, method, evidence, limits, open questions, cross-refs. |
| [`wiki/concepts/superposition.md`](../../domains/research-papers/wiki/concepts/superposition.md), [`sparse-autoencoder.md`](../../domains/research-papers/wiki/concepts/sparse-autoencoder.md) | Two evergreen concept pages citing the summary and each other. |
| [`wiki/overviews/mechanistic-interpretability.md`](../../domains/research-papers/wiki/overviews/mechanistic-interpretability.md) | A `type: overview` page surveying the research programme. |
| [`wiki/open-questions/saes-cross-model-transfer.md`](../../domains/research-papers/wiki/open-questions/saes-cross-model-transfer.md) | An `type: open-question` page — tracks evidence across multiple summaries over time. |
...
— it braids the Vanzo / Bastani / Kestin summaries into one evidence
arc, then layers the Kim 2025 review on top, with shared open
questions
([`llm-tutoring-cognitive-offload`](../../domains/research-papers/wiki/open-questions/llm-tutoring-cognitive-offload.md),
[`llm-tutoring-equity-impact`](../../domains/research-papers/wiki/open-questions/llm-tutoring-equity-impact.md)).
...
flowchart LR
    ...
    SUM["wiki/summaries/<br/>...-summary"]
    C1["wiki/concepts/<br/>superposition"]
    C2["wiki/concepts/<br/>sparse-autoencoder"]
    OV["wiki/overviews/<br/>mechanistic-interpretability"]
    Q["wiki/open-questions/<br/>saes-cross-model-transfer"]
    SUM -->|sources| RAW
    C1 -->|cites| SUM
    C2 -->|cites| SUM
    OV -->|cites| SUM
...
1. **Every wiki claim traces back to raw in ≤ 2 hops.** The summary
   cites raw directly; second-order pages (concepts, overviews,
   open-questions) cite the summary. Lint flags chains longer than that.
...
3. **Open questions are first-class.** Instead of a TODO list buried
   in a single summary, `saes-cross-model-transfer` lives in
   `wiki/open-questions/` and accumulates evidence rows over time.
...
| File | Why it's interesting |
| --- | --- |
| [`AGENTS.md`](psychology/AGENTS.md) | The L2 schema — read for the upper bound on L2 complexity. |
