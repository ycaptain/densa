...
> `python -m wikilint --staged`) already enforces raw immutability,
...
8. **Auto-apply** only additive, low-risk fixes:
   ...
   - **Generate Dataview snapshot**: lint runs MUST emit
     `outputs/snapshots/index-snapshot.md` (create the directory if it
     doesn't exist). The file is a static mirror of every Dataview /
     dataviewjs block in [`index.md`](../../index.md) and each
     `domains/<X>/index.md`, replaced with the markdown list/table
     they would currently render. This gives a fresh-session LLM a
     readable page inventory, since LLMs cannot execute Dataview.

     Implementation (the LLM executes this itself):
     - Read `index.md` and every `domains/<X>/index.md`. Identify each
       ` ```dataview ` and ` ```dataviewjs ` fenced block.
     - For each block, run an equivalent query inside the repo with
       `rg` / `git grep` / Python over the YAML frontmatter (e.g.
       `FROM "domains" AND #cross-domain WHERE type = "synthesis" AND
       status = "active"` ≡ enumerate all wiki `.md` whose frontmatter
       `tags` contains `cross-domain`, `type` is `synthesis`, and
       `status` is `active`).
     - Render the result as a static markdown table or bullet list and
       splice it into the snapshot file at the same position the
       original code fence occupied (drop the fence; keep the
       surrounding prose).
     - Snapshot frontmatter:
       ...
       ---
       ```
     - Prepend a callout banner at the top of the body:
       ```
       > [!warning] Auto-generated by lint
       > Do not edit by hand. Canonical source is `index.md`. LLMs in
       > new sessions should read this file to get the actual page
       > inventory, since they cannot execute Dataview queries.
       ```

     This step is `auto-applied` (no human review needed); regenerate
     on every lint run.

     This snapshot is the file referenced by `/AGENTS.md` §1.1 step 4 as
     part of the minimal LLM onboarding set. Keep it always present and
     fresh — its absence forces every new LLM session to fall back to
     reading `index.md` source code and missing the actual page inventory.
