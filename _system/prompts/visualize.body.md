...
| debrief                                | client-voice mechanism loop diagrams (2nd person, CFT tone, plain-language gloss, ≤6 nodes each) | encouraged: default when a mechanism loop has an observation homework (H-ID) attached; others as needed — no per-page cap, but every chart independently passes the (z2) shape test (CBT formulation diagrams are client-facing tools by tradition) | inside that mechanism block, collapsed |
